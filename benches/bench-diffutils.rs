...
#[cfg(feature = "bench_cmp")]
mod diffutils_cmp {
    use std::hint::black_box;

    use diffutilslib::cmp::{
        self,
        params_cmp::{ParamsCmp, ParamsCmpOk},
    };
    use divan::Bencher;
    ...
    fn diff_compare_files_equal(bencher: Bencher, lines: usize) {
        ...
        let params_ok = ParamsCmp::parse_params(opts).unwrap();
        let params = match params_ok {
            ParamsCmpOk::Info(_params_cmp_info) => todo!("params returned --version or --help"),
            ParamsCmpOk::ParamsCmp(params_cmp) => params_cmp,
        };
        bencher
...
#[cfg(feature = "bench_parser")]
mod parser {
    use std::hint::black_box;

    use diffutilslib::{cmp::params_cmp::ParamsCmp, params};
    use divan::Bencher;
    ...
    fn cmp_parser(bencher: Bencher) {
        ...
        bencher
            ...
            .bench_values(|data| black_box(ParamsCmp::parse_params(data)));
    }

    // bench the time it takes to parse the command line arguments
    #[divan::bench]
    fn cmp_parser_org(bencher: Bencher) {
        let cmd = "cmd file_1.txt file_2.txt -bl n10M --ignore-initial=100KiB:1MiB";
        let args = str_to_options(&cmd).into_iter().peekable();
        bencher
            .with_inputs(|| args.clone())
            .bench_values(|data| black_box(diffutilslib::cmp_org::parse_params(data)));
    }
    ...
    fn cmp_parser_no_prepare() {
        let cmd = "cmd file_1.txt file_2.txt -bl n10M --ignore-initial=100KiB:1MiB";
        let args = str_to_options(&cmd).into_iter().peekable();
        let _ = ParamsCmp::parse_params(args);
    }
