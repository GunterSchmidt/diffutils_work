...
use crate::params_diff3::{Diff3ParseOk, ParamsDiff3};
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    match diff3(args) {
        ...
        Err(e) => {
            ...
            eprintln!("{msg}");
            uucore::error::set_exit_code(2) // ExitCode::from(2)
        }
    }
...
pub fn diff3<I: Iterator<Item = OsString>>(
    mut args: Peekable<I>,
) -> Result<DiffThreeOk, Diff3Error> {
    let Some(executable) = Executable::from_args_os(&mut args, false) else {
    ...
    let params = match ParamsDiff3::parse_params(&executable, args)? {
        ...
        Diff3ParseOk::Help => return Ok(DiffThreeOk::Help),
        Diff3ParseOk::Version => return Ok(DiffThreeOk::Version),
    };
    // dbg!("{params:?}");
...
#[allow(unused)] // until compare is written
pub fn diff3_compare(params: &ParamsDiff3) -> Result<DiffThreeOk, Diff3Error> {
    // Dummy, this just compares 2 instead of 3 files
    if utils::is_same_file(&params.from, &params.to) {
        return Ok(DiffThreeOk::Equal);
    }
